...
def remove_subuser(request: HttpRequest, id):
    ...
    if check_user_and_usersub:
        ...
        if not check_insurance:
            ...
            messages.error(request, "این کاربر بیمه نامه فعال دارد")
...
def remove_insurance(request, code):
    if code:
        get_insurance = get_object_or_404(Insurance, code=code)

        if get_insurance.pay_status == "0":
            get_insurance.delete()
            messages.success(request, "بیمه نامه حذف شد")
